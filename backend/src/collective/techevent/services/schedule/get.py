...
def group_slots(slots: list[dict], rooms_vocab: dict[str, str]) -> list[dict]:
    ...
    for day in response:
        ...
        for slot in day["items"]:
            ...
            # Collect rooms for the day
            if room_tokens:
                for token in room_tokens:
                    rooms.add(token)
            else:
                rooms.add("_all_")  # Use special token for slots without a room
        ...
        for slot in day["items"]:
            ...
            if room_tokens:
                ...
                slot["gridColumn"] = "room-1 / all-rooms"
